...
export const LoginUser = createAsyncThunk('signin', async (userData, { rejectWithValue }) => {
  ...
  try {
    const response = await axios.post('https://motorbikebookingbackend-production.up.railway.app/login', { user: userData });
    ...
    toast.update(id, {
      render: err[0], type: 'error', autoClose: 2000, isLoading: false,
    });
...
const signinSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(LoginUser.fulfilled, (state, action) => {
      state.loading = false; state.user = [action.payload.user]; state.errors = [];
    });
    builder.addCase(LoginUser.rejected, (state, action) => {
      state.loading = false; state.user = []; state.errors = action.payload;
    });
  },
});
