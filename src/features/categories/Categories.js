import React from 'react';
import './Categories.css';

const Categories = () => (
  <div className="">
    <div className="header-title">
      <h5 className="d-flex justify-content-center"><strong>MOTORBIKE BRANDS</strong></h5>
      <h6 className="d-flex justify-content-center text-secondary"><strong>Please Select any Brand</strong></h6>
    </div>
    <div className="container">
      <ul className="grid-container list-unstyled">
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://ridermagazine.com/wp-content/uploads/2022/10/vespa-gts-super-300-in-Rosso-Coraggioso.jpg" alt="Vespa motorbike" />
          </div>
          <div className="model-details">
            <p><strong>VESPA</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://kservico.com.ph/wp-content/uploads/2017/10/nmax-icon-gray-std-600x600.png" alt="Yamaha motorbike" />
          </div>
          <div className="model-details">
            <p><strong>YAMAHA</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://cdp.azureedge.net/products/USA/SU/2020/SCOOTER/SCOOTER/BURGMAN_200/50/WHITE/2000000001.jpg" alt="suzuki motorbike" />
          </div>
          <div className="model-details">
            <p><strong>SUZUKI</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://ridermagazine.com/wp-content/uploads/2022/10/vespa-gts-super-300-in-Rosso-Coraggioso.jpg" alt="Vespa motorbike" />
          </div>
          <div className="model-details">
            <p><strong>VESPA</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://kservico.com.ph/wp-content/uploads/2017/10/nmax-icon-gray-std-600x600.png" alt="Yamaha motorbike" />
          </div>
          <div className="model-details">
            <p><strong>YAMAHA</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://cdp.azureedge.net/products/USA/SU/2020/SCOOTER/SCOOTER/BURGMAN_200/50/WHITE/2000000001.jpg" alt="suzuki motorbike" />
          </div>
          <div className="model-details">
            <p><strong>SUZUKI</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://kservico.com.ph/wp-content/uploads/2017/10/nmax-icon-gray-std-600x600.png" alt="Yamaha motorbike" />
          </div>
          <div className="model-details">
            <p><strong>YAMAHA</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
        <li>
          <div className="model-img div-height">
            <img className="brand-img" src="https://cdp.azureedge.net/products/USA/SU/2020/SCOOTER/SCOOTER/BURGMAN_200/50/WHITE/2000000001.jpg" alt="suzuki motorbike" />
          </div>
          <div className="model-details">
            <p><strong>SUZUKI</strong></p>
            <div className="border-top1 my-3 d-flex justify-content-center" />
            <p className="text-secondary descrptive-text">
              Brand details dffdfdfdsd fhdfgfdyf dfgddfdgdff fgddfd dggdfg ydfgdf
            </p>
            <div className="social-media">
              <div>
                <a href="https://www.facebook.com/">
                  <img
                    className="soc-img"
                    src="https://t3.ftcdn.net/jpg/03/21/87/72/360_F_321877261_22BLEJLx609sAdV9VFphDxHxSl5JNfNG.jpg"
                    alt="linkedin"
                  />
                </a>
                <a href="https://www.twitter.com/">
                  <img
                    className="soc-img"
                    src="https://www.witness.org/wp-content/uploads/2018/11/5a2fe479cc45e43754640849.png"
                    alt="Twitter"
                  />
                </a>
                <a href="https://www.instagram.com/">
                  <img
                    className="soc-img"
                    src="https://hippocketwifi.com/wp-content/uploads/2016/11/instagram.png"
                    alt="instagram"
                  />
                </a>
              </div>
            </div>
          </div>
        </li>
      </ul>
    </div>
  </div>
);

export default Categories;
